...
class RecMetric(object):
    ...
    def __call__(self, post_result, batch, *args, **kwargs):
        ...
        for (pred, pred_conf), (target, _) in zip(preds, labels):
            ...
            import re
            pred = re.sub(r'[^A-Z0-9]', '', str(pred).upper())
            target = re.sub(r'[^A-Z0-9]', '', str(target).upper())
            if pred == target:
                correct_num += 1
            else:
                # This is the error logging part
                img_path = "N/A"
                # The batch is a dict in training, but a list/tuple in evaluation
                if isinstance(batch, dict) and "img_path" in batch:
                    img_path = batch["img_path"][all_num]
                elif isinstance(batch, (list, tuple)) and len(batch) > 3:
                    # In eval, batch is a list of numpy arrays. img_path is typically the last element.
                    img_path_array = batch[-1]
                    if all_num < len(img_path_array):
                        img_path = img_path_array[all_num]
                logger.info(f"FAIL | GT: '{target}' | PRED: '{pred}'")
            all_num += 1
...
class CANMetric(object):
    ...
    def __call__(self, preds, batch, **kwargs):
        epoch_reset = kwargs.get('epoch_reset', False)
        if epoch_reset:
            self.epoch_reset()
        word_probs = preds
...
class LaTeXOCRMetric(object):
    ...
    def __call__(self, preds, batch, **kwargs):
        ...
        if epoch_reset:
            self.epoch_reset()
        word_pred = preds
        # The batch from the dataloader is often a tuple (image, labels, ...).
        # We need the labels, which are typically the second element.
        # If batch is not a tuple/list, assume it's just the labels.
