...
def get_image_file_list(img_file, infer_list=None):
    ...
    # Try to resolve path relative to project root if it doesn't exist from CWD
    if not os.path.exists(img_file):
        # Assuming this script is in PaddleOCR/ppocr/utils, project root is 3 levels up
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
        candidate_path = os.path.join(project_root, img_file)
        if os.path.exists(candidate_path):
            img_file = candidate_path
            # print(f"Path not found, but resolved to: {img_file}") # for debugging
